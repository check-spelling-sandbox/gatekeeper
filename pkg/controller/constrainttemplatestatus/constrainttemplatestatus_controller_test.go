...
const (
	...
	constraintTemplateName = "denyall"
...
func TestReconcile(t *testing.T) {
	...
	template := &v1beta1.ConstraintTemplate{
		ObjectMeta: metav1.ObjectMeta{Name: constraintTemplateName},
...
`,
	...
	t.Run("Multiple constraint template statuses are reported", func(t *testing.T) {
		fakeTStatus, err := podstatus.NewConstraintTemplateStatusForPod(fakePod, constraintTemplateName, mgr.GetScheme())
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Invalid ConstraintTemplate has .status.created set to false", func(t *testing.T) {
		g.Eventually(verifyTStatusCount(ctx, c, 0), timeout).Should(gomega.BeNil())
		err := c.Create(ctx, templateCpy)
		if err != nil {
			t.Fatal(err)
...
func verifyTStatusCreated(ctx context.Context, c client.Client, expected bool) func() error {
	return func() error {
		...
		if err := c.Get(ctx, types.NamespacedName{Name: constraintTemplateName}, ct); err != nil {
			return err
		}
...
func newDenyAllConstraint() *unstructured.Unstructured {
	...
	constraint.SetName(constraintTemplateName + "constraint")
	return constraint
}
